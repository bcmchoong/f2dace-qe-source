...
SUBROUTINE gcxc( length, rho_in, grho_in, sx_out, sc_out, v1x_out, &
  ...
  REAL(DP) :: sc, v1c, v2c
  !
!# 84 "qe_drivers_gga.f90"
  !
  err_out = 0
  !
!# 96 "qe_drivers_gga.f90"
...
!# 105 "qe_drivers_gga.f90"
  DO ir = 1, length  
     ...
     !
     IF (in_err/=0) THEN
!# 447 "qe_drivers_gga.f90"
!$omp atomic write
!# 449 "qe_drivers_gga.f90"
       err_out = in_err
     ENDIF
     !
     ...
     !
  ENDDO
!# 460 "qe_drivers_gga.f90"
!$omp end do
!$omp end parallel
!# 463 "qe_drivers_gga.f90"
  !
  RETURN
  !
END SUBROUTINE gcxc
...
SUBROUTINE gcx_spin( length, rho_in, grho2_in, sx_tot, v1x_out, v2x_out, err_out )
  ...
  !
!# 519 "qe_drivers_gga.f90"
  !
  err_out = 0
  !
!# 530 "qe_drivers_gga.f90"
!$omp parallel if(ntids==1) default(none) &
...
!# 540 "qe_drivers_gga.f90"
  DO ir = 1, length  
     ...
     !
     IF (in_err/=0) THEN
!# 1019 "qe_drivers_gga.f90"
!$omp atomic write
!# 1021 "qe_drivers_gga.f90"
       err_out = in_err
     ENDIF
     !
     ...
     !
  ENDDO
!# 1033 "qe_drivers_gga.f90"
!$omp end do
!$omp end parallel
!# 1036 "qe_drivers_gga.f90"
  !
  RETURN
  !
...
SUBROUTINE gcc_spin( length, rho_in, zeta_io, grho_in, sc_out, v1c_out, v2c_out )
  ...
  !
!# 1082 "qe_drivers_gga.f90"
  !
!# 1087 "qe_drivers_gga.f90"
!$omp parallel if(ntids==1) default(none) &
...
!# 1094 "qe_drivers_gga.f90"
  DO ir = 1, length
    ...
    !
  ENDDO
!# 1149 "qe_drivers_gga.f90"
!$omp end do
!$omp end parallel
!# 1152 "qe_drivers_gga.f90"
  !
  RETURN
  !
...
SUBROUTINE gcc_spin_more( length, rho_in, grho_in, grho_ud_in, &
  ...
  REAL(DP) :: grho_ud
!# 1206 "qe_drivers_gga.f90"
  !
!# 1211 "qe_drivers_gga.f90"
!$omp parallel if(ntids==1) default(none) &
...
!# 1218 "qe_drivers_gga.f90"
  DO ir = 1, length
    ...
    !
  ENDDO
!# 1275 "qe_drivers_gga.f90"
!$omp end do
!$omp end parallel
!# 1278 "qe_drivers_gga.f90"
  !
  RETURN
  !
