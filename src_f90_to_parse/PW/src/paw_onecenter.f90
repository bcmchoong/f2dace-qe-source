...
!# 548 "paw_onecenter.f90"
  ...
  SUBROUTINE PAW_gcxc_potential( i, rho_lm, rho_core, v_lm, energy )
    ...
    !
!# 737 "paw_onecenter.f90"
    !$omp parallel shared(gc_rad,h_rad)
    !$omp workshare
!# 740 "paw_onecenter.f90"
    gc_rad = 0.0d0
    h_rad  = 0.0d0
!# 745 "paw_onecenter.f90"
    !$omp end workshare
    !$omp end parallel
!# 748 "paw_onecenter.f90"
    !
    IF ( energy_present ) THEN
      ALLOCATE( e_rad(im_sum) )
      !$acc enter data create( e_rad )
