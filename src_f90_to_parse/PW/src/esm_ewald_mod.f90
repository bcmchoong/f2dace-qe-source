...
!# 41 "esm_ewald_mod.f90"
    charge = 0.d0
    DO na = 1, nat
      charge = charge + zv(ityp(na))
    ENDDO
!# 46 "esm_ewald_mod.f90"
...
!# 126 "esm_ewald_mod.f90"
    ...
    !
    ewr = 0.d0
!# 152 "esm_ewald_mod.f90"
    tmp = sqrt(alpha_g)
    rmax0 = 4.d0/tmp/alat
!# 155 "esm_ewald_mod.f90"
    ip = mp_rank(intra_bgrp_comm)
    np = mp_size(intra_bgrp_comm)
!# 158 "esm_ewald_mod.f90"
    ew = 0.d0
    DO na = ip + 1, nat, np
      DO nb = 1, nat
        dtau(:) = tau(:, na) - tau(:, nb)
...
!# 193 "esm_ewald_mod.f90"
    ...
    IF (iverbosity > 0) THEN
      ...
      write (stdout, '(5x,"===========================================")')
    ENDIF
!# 273 "esm_ewald_mod.f90"
    ip = mp_rank(intra_bgrp_comm)
    np = mp_size(intra_bgrp_comm)
!# 276 "esm_ewald_mod.f90"
    ew = 0.d0
    DO na = ip + 1, nat, np
      z = tau(3, na)
...
!# 378 "esm_ewald_mod.f90"
    IMPLICIT NONE
    REAL(DP), INTENT(in)  :: alpha_g
    REAL(DP), INTENT(out) :: ewg
!# 382 "esm_ewald_mod.f90"
    INTEGER     :: ng
    REAL(DP)    :: charge, fact
    COMPLEX(DP) :: rhon
!# 386 "esm_ewald_mod.f90"
    charge = sum(zv(ityp(1:nat)))
!# 388 "esm_ewald_mod.f90"
...
!# 470 "esm_ewald_mod.f90"
    ...
    ENDDO
    ewg = ewg + ew
!# 480 "esm_ewald_mod.f90"
    RETURN
  END SUBROUTINE esm_ewaldg_bc1
!# 483 "esm_ewald_mod.f90"
  SUBROUTINE esm_ewaldg_bc2(alpha_g, ewg)
!# 485 "esm_ewald_mod.f90"
    USE constants,        ONLY : pi, tpi, fpi, e2
    USE gvect,            ONLY : gstart
    USE cell_base,        ONLY : omega, alat, tpiba2, at, bg
    USE ions_base,        ONLY : zv, nat, nsp, ityp, tau
    USE control_flags,    ONLY : gamma_only
!# 491 "esm_ewald_mod.f90"
    IMPLICIT NONE
    REAL(DP), INTENT(in)  :: alpha_g
    REAL(DP), INTENT(out) :: ewg
    !
...
!# 520 "esm_ewald_mod.f90"
        IF (gstart == 2) THEN
          ...
          END IF
        END IF
!# 527 "esm_ewald_mod.f90"
        ! bc2
        arg001 = -tmp**2*(z - zp)**2
        arg101 = tmp*(z - zp)
        kk1 = 0.5d0*(-(z - zp)*erf(arg101) - EXP(arg001)/tmp/sqrt(pi))
        kk2 = 0.5d0*(z1 - z*zp/z1)
!# 533 "esm_ewald_mod.f90"
        cc1 = 0.d0
        cc2 = 0.d0
        DO ng_2d = 1, ngm_2d
          k1 = mill_2d(1, ng_2d)
...
!# 562 "esm_ewald_mod.f90"
    ...
    ENDDO
    ewg = ewg + ew
!# 572 "esm_ewald_mod.f90"
    RETURN
  END SUBROUTINE esm_ewaldg_bc2
!# 575 "esm_ewald_mod.f90"
  SUBROUTINE esm_ewaldg_bc3(alpha_g, ewg)
!# 577 "esm_ewald_mod.f90"
    USE constants,        ONLY : pi, tpi, fpi
    USE gvect,            ONLY : gstart
    USE cell_base,        ONLY : omega, alat, tpiba2, at, bg
    USE ions_base,        ONLY : zv, nat, nsp, ityp, tau
    USE control_flags,    ONLY : gamma_only
!# 583 "esm_ewald_mod.f90"
    IMPLICIT NONE
    REAL(DP), INTENT(in)  :: alpha_g
    REAL(DP), INTENT(out) :: ewg
    !
...
!# 639 "esm_ewald_mod.f90"
    ...
    ENDDO
    ewg = ewg + ew
!# 649 "esm_ewald_mod.f90"
    RETURN
  END SUBROUTINE esm_ewaldg_bc3
!# 652 "esm_ewald_mod.f90"
  SUBROUTINE esm_ewaldg_bc4(alpha_g, ewg)
!# 654 "esm_ewald_mod.f90"
    USE constants,        ONLY : pi, tpi, fpi
    USE gvect,            ONLY : gstart
    USE cell_base,        ONLY : omega, alat, tpiba2, at, bg
    USE ions_base,        ONLY : zv, nat, nsp, ityp, tau
    USE control_flags,    ONLY : gamma_only
!# 660 "esm_ewald_mod.f90"
    IMPLICIT NONE
    REAL(DP), INTENT(in)  :: alpha_g
    REAL(DP), INTENT(out) :: ewg
    !
...
!# 766 "esm_ewald_mod.f90"
    ...
    ENDDO
    ewg = ewg + ew
!# 776 "esm_ewald_mod.f90"
    RETURN
  END SUBROUTINE esm_ewaldg_bc4
!# 779 "esm_ewald_mod.f90"
