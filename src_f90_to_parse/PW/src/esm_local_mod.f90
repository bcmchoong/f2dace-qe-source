...
!# 15 "esm_local_mod.f90"
    ...
    END IF
!# 27 "esm_local_mod.f90"
  END SUBROUTINE esm_local
!# 29 "esm_local_mod.f90"
!-----------------------------------------------------------------------
...
!# 144 "esm_local_mod.f90"
      ...
      DO iz = 1, dfftp%nr3
        vloc3(iz, ng_2d) = vg(iz)
      ENDDO
!# 176 "esm_local_mod.f90"
      DEALLOCATE (vg, vg_r)
    ENDIF ! IF( ng_2d > 0 )
!# 179 "esm_local_mod.f90"
! Map to FFT mesh (dfftp%nrx)
    ...
    ENDDO
!# 189 "esm_local_mod.f90"
    DEALLOCATE (vloc3)
!# 191 "esm_local_mod.f90"
    RETURN
  END SUBROUTINE esm_local_bc1
!# 194 "esm_local_mod.f90"
  SUBROUTINE esm_local_bc2(aux)
!# 196 "esm_local_mod.f90"
    USE constants,        ONLY : pi, tpi, fpi, e2
    USE gvect,            ONLY : ngm, mill
    USE control_flags,    ONLY : gamma_only
    USE cell_base,        ONLY : at, bg, alat, tpiba2, omega
...
!# 322 "esm_local_mod.f90"
      ...
      DO iz = 1, dfftp%nr3
        vloc3(iz, ng_2d) = vg(iz)
      ENDDO
!# 358 "esm_local_mod.f90"
      DEALLOCATE (vg, vg_r)
    ENDIF ! IF( ng_2d > 0 )
!# 361 "esm_local_mod.f90"
! Map to FFT mesh (dfftp%nrx)
    DO ng = 1, ngm
      n1 = mill(1, ng)
      n2 = mill(2, ng)
      ...
      aux(ng) = aux(ng) + vloc3(n3,ng_2d)
    ENDDO
!# 371 "esm_local_mod.f90"
    DEALLOCATE (vloc3)
!# 373 "esm_local_mod.f90"
    RETURN
  END SUBROUTINE esm_local_bc2
!# 376 "esm_local_mod.f90"
  SUBROUTINE esm_local_bc3(aux)
!# 378 "esm_local_mod.f90"
    USE constants,        ONLY : pi, tpi, fpi, e2
    USE gvect,            ONLY : ngm, mill
    USE control_flags,    ONLY : gamma_only
    USE cell_base,        ONLY : at, bg, alat, tpiba2, omega
...
!# 483 "esm_local_mod.f90"
      ...
      DO iz = nz_r, nz_l
        ...
        vg_r(iz) = (a0 + a1*z + a2*z**2 + a3*z**3)
      ENDDO
!# 515 "esm_local_mod.f90"
      CALL cft_1z(vg_r, 1, dfftp%nr3, dfftp%nr3, -1, vg)
      DO iz = 1, dfftp%nr3
        vloc3(iz, ng_2d) = vg(iz)
      ENDDO
!# 520 "esm_local_mod.f90"
      DEALLOCATE (vg, vg_r)
    ENDIF ! IF( ng_2d > 0 )
!# 523 "esm_local_mod.f90"
! Map to FFT mesh (dfftp%nrx)
    DO ng = 1, ngm
      n1 = mill(1, ng)
      n2 = mill(2, ng)
      ...
      aux(ng) = aux(ng) + vloc3(n3,ng_2d)
    ENDDO
!# 533 "esm_local_mod.f90"
    DEALLOCATE (vloc3)
!# 535 "esm_local_mod.f90"
    RETURN
  END SUBROUTINE esm_local_bc3
!# 538 "esm_local_mod.f90"
  SUBROUTINE esm_local_bc4(aux)
!# 540 "esm_local_mod.f90"
    USE constants,        ONLY : pi, tpi, fpi, e2
    USE gvect,            ONLY : ngm, mill
    USE control_flags,    ONLY : gamma_only
    USE cell_base,        ONLY : at, bg, alat, tpiba2, omega
...
!# 695 "esm_local_mod.f90"
      ...
      DO iz = nz_r, nz_l
        ...
        vg_r(iz) = (a0 + a1*z + a2*z**2 + a3*z**3)
      ENDDO
!# 741 "esm_local_mod.f90"
      CALL cft_1z(vg_r, 1, dfftp%nr3, dfftp%nr3, -1, vg)
      DO iz = 1, dfftp%nr3
        vloc3(iz, ng_2d) = vg(iz)
      ENDDO
!# 746 "esm_local_mod.f90"
      DEALLOCATE (vg, vg_r)
    ENDIF ! IF( ng_2d > 0 )
!# 749 "esm_local_mod.f90"
! Map to FFT mesh (dfftp%nrx)
    DO ng = 1, ngm
      n1 = mill(1, ng)
      n2 = mill(2, ng)
      ...
      aux(ng) = aux(ng) + vloc3(n3,ng_2d)
    ENDDO
!# 759 "esm_local_mod.f90"
    DEALLOCATE (vloc3)
!# 761 "esm_local_mod.f90"
    RETURN
