!# 1 "esm_common_mod.f90"
MODULE esm_common_mod
!# 3 "esm_common_mod.f90"
  USE kinds, ONLY : DP
  IMPLICIT NONE
!# 6 "esm_common_mod.f90"
  INTEGER                  :: esm_nfit
...
!# 13 "esm_common_mod.f90"
CONTAINS
!# 15 "esm_common_mod.f90"
...
!# 177 "esm_common_mod.f90"
    RETURN
  END SUBROUTINE esm_ggen_2d
!# 180 "esm_common_mod.f90"
!-----------------------------------------------------------------------
...
!
  SUBROUTINE esm_printpot(rhog)
    ...
    IMPLICIT NONE
!# 201 "esm_common_mod.f90"
    COMPLEX(DP), INTENT(in) :: rhog(ngm)   !  n(G)
!# 203 "esm_common_mod.f90"
...
!# 215 "esm_common_mod.f90"
    IF (imill_2d(0, 0) == 0) RETURN
!# 217 "esm_common_mod.f90"
    !****For gp=0 case ********************
!# 219 "esm_common_mod.f90"
    ! cell settings
    L = at(3, 3)*alat
    S = omega/L
    z0 = L/2.d0
...
!# 231 "esm_common_mod.f90"
    rho0g(:) = (0.d0, 0.d0)
!# 233 "esm_common_mod.f90"
    !!---- calculate density potential
    DO ig = 1, ngm
      iga = mill(1, ig)
      igb = mill(2, ig)
      igz = mill(3, ig) + 1
!# 239 "esm_common_mod.f90"
      IF (.not. (iga == 0 .and. igb == 0)) CYCLE
!# 241 "esm_common_mod.f90"
      IF (igz < 1) THEN
        igz = igz + dfftp%nr3
      END IF
!# 245 "esm_common_mod.f90"
      rg3 = rhog(ig)
      rho0g(igz) = rg3
!# 248 "esm_common_mod.f90"
...
!# 315 "esm_common_mod.f90"
    ...
    DO ig = 1, ngm
      iga = mill(1, ig)
      igb = mill(2, ig)
!# 322 "esm_common_mod.f90"
      IF (.not. (iga == 0 .and. igb == 0)) CYCLE
!# 324 "esm_common_mod.f90"
      igz = mill(3, ig) + 1
      IF (igz < 1) THEN
        igz = igz + dfftp%nr3
      END IF
!# 329 "esm_common_mod.f90"
      vg3 = 0.0d0
      DO ia = 1, ntyp
        vg3 = vg3 + vloc(igtongl(ig), ia)*strf(ig, ia)/e2
      END DO
      Vloc0g(igz) = vg3
!# 335 "esm_common_mod.f90"
      IF (gamma_only .and. iga == 0 .and. igb == 0) THEN
        igz = 1 - mill(3, ig)
        IF (igz < 1) THEN
          igz = igz + dfftp%nr3
        END IF
!# 341 "esm_common_mod.f90"
        Vloc0g(igz) = CONJG(vg3)
      END IF
    END DO
!# 345 "esm_common_mod.f90"
    CALL cft_1z(Vloc0g, 1, dfftp%nr3, dfftp%nr3, +1, Vloc0r)
!# 347 "esm_common_mod.f90"
    ! long range
    DO iz = 1, dfftp%nr3
      jz = iz - 1
      IF (jz >= (dfftp%nr3 - dfftp%nr3/2)) THEN
      ...
      END IF
      z = DBLE(jz) / DBLE(dfftp%nr3) * L
!# 355 "esm_common_mod.f90"
      IF (esm_bc == 'bc2') THEN
        Vloc0r(iz) = Vloc0r(iz) + (z1 - z)*esm_efield/e2
      END IF
!# 359 "esm_common_mod.f90"
      DO ia = 1, nat
        Qa = (-1.0d0)*zv(ityp(ia))
...
! This function is to avoid INFINITY * ZERO for large positive x and y.
  REAL(8) FUNCTION exp_erfc(x, y)
    ...
    !END IF
!# 589 "esm_common_mod.f90"
    exp_erfc = exp(x + log(erfc(y)))
!# 591 "esm_common_mod.f90"
    RETURN
  END FUNCTION exp_erfc
!# 594 "esm_common_mod.f90"
