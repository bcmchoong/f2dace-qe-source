...
SUBROUTINE extrapolate_wfcs( wfc_extr )
  ...
  ELSE 
     ...
     DO ik = 1, nks
        ...
        !
        CALL ZGEMM( 'N', 'N', nbnd, nbnd, nbnd, (1.D0,0.D0), &
                    u_m, nbnd, w_m, nbnd, (0.D0,0.D0), sp_m, nbnd )
        !
        ! ... now use aux as workspace to calculate "aligned" wavefcts:
        !
        ! ... aux_i = sum_j evcold_j*s_m_ji (eq.3.21)
        !
        CALL ZGEMM( 'N', 'C', npw, nbnd, nbnd, (1.D0,0.D0), &
                    evcold, npwx, sp_m, nbnd, (0.D0,0.D0), aux, npwx )
        !
