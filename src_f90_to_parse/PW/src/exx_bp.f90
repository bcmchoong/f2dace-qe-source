...
!$omp end parallel do
                ...
                IF (okvan .AND. tqr) THEN
                   IF (jbnd >= jstart) &
                       ...
                       CMPLX(becxx(ikq)%r(:,jbnd),0._dp,kind=DP), CMPLX(becpsi%r(:,ibnd),0._dp,kind=DP))
                   IF (jbnd < jend) &
                        CALL addusxx_r( psi_rhoc_work, &
                       CMPLX(0._dp,-becxx(ikq)%r(:,jbnd+1),kind=DP),CMPLX(becpsi%r(:,ibnd),0._dp,kind=DP))
                ENDIF
                !
...
!$omp end parallel do
                ...
                IF (okvan .AND. tqr) THEN
                   IF (jbnd >= jstart) &
                        CALL newdxx_r( dfftt,vc, CMPLX(x1*becxx(ikq)%r(:,jbnd),0._dp,kind=DP), deexx(:,ii) )
                   IF (jbnd < jend) &
                        CALL newdxx_r( dfftt,vc, CMPLX(0._dp,-x2*becxx(ikq)%r(:,jbnd+1),kind=DP), deexx(:,ii) )
                ENDIF
                !
                IF (okpaw) THEN
                   IF (jbnd >= jstart) &
                        CALL PAW_newdxx( x1/nqs, CMPLX(becxx(ikq)%r(:,jbnd),0._dp,kind=DP),&
                                                CMPLX(becpsi%r(:,ibnd),0._dp,kind=DP), deexx(:,ii) )
                   IF (jbnd < jend) &
                        CALL PAW_newdxx( x2/nqs, CMPLX(becxx(ikq)%r(:,jbnd+1),0._dp,kind=DP),&
                                                CMPLX(becpsi%r(:,ibnd),0._dp,kind=DP), deexx(:,ii) )
                ENDIF
                !
...
!$omp end parallel do
                   ...
                   IF (okvan .AND. tqr) THEN
                      IF (ibnd >= istart) &
                           CALL addusxx_r( rhoc,CMPLX(becxx(ikq)%r(:,ibnd),0._dp,kind=DP), &
                                          CMPLX(becpsi%r(:,jbnd),0._dp,kind=DP) )
                      IF (ibnd<iend) &
                           CALL addusxx_r(rhoc,CMPLX(0._dp,-becxx(ikq)%r(:,ibnd+1),kind=DP), &
                                          CMPLX(becpsi%r(:,jbnd),0._dp,kind=DP))
                   ENDIF
                   !
...
!$omp end parallel do
                   ...
                   IF (okpaw) THEN
                      IF (ibnd >= ibnd_start) &
                           ...
                           x1 * PAW_xx_energy(CMPLX(becxx(ikq)%r(:,ibnd),0._dp,kind=DP),CMPLX(becpsi%r(:,jbnd),0._dp,kind=DP) )
                      IF (ibnd < ibnd_end) &
                           energy = energy + exxalfa*wg(jbnd,ikk)*&
                           x2 * PAW_xx_energy(CMPLX(becxx(ikq)%r(:,ibnd+1),0._dp,kind=DP), CMPLX(becpsi%r(:,jbnd),0._dp,kind=DP) )
                   ENDIF
                   !
                ENDDO &
                IBND_LOOP_GAM
