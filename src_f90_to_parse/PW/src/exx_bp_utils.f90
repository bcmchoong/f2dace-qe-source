...
!# 775 "exx_bp_utils.f90"
    ...
    !
    DO iproc=0, nproc_egrp-1
!# 784 "exx_bp_utils.f90"
      !
      !prepare receive buffers
      !
      IF ( comm_recv(iproc+1,current_ik)%size.gt.0) THEN
!# 808 "exx_bp_utils.f90"
         !
      END IF
!# 811 "exx_bp_utils.f90"
      !
...
!# 936 "exx_bp_utils.f90"
  ...
  SUBROUTINE change_data_structure( is_exx )
    ...
    LOGICAL exst
!# 981 "exx_bp_utils.f90"
  LOGICAL :: lpara = .false.
!# 983 "exx_bp_utils.f90"
    !
...
!# 1125 "exx_bp_utils.f90"
    ...
    ELSE ! not is_exx
       ...
       dfftp%nl = nl_loc
       dffts%nl = nls_loc
!# 1144 "exx_bp_utils.f90"
       IF( gamma_only ) THEN
          dfftp%nlm = nlm_loc
          dffts%nlm = nlsm_loc
!# 1151 "exx_bp_utils.f90"
       END IF
       ngm = ngm_loc
       ngm_g = ngm_g_loc
       gstart = gstart_loc
...
!# 1250 "exx_bp_utils.f90"
    INTEGER, INTENT(in) :: n, m
    COMPLEX(DP), INTENT(inout) :: data(n,m)
!# 1263 "exx_bp_utils.f90"
    !
    IF (negrp.eq.1) RETURN
    !
!# 1337 "exx_bp_utils.f90"
    !
    ...
    DO iegrp=1, negrp
       ...
       DO iproc=0, nproc_egrp-1
          IF ( comm_recv_reverse(iproc+1,current_ik)%size.gt.0) THEN
             ...
             !
             tag = 0
!# 1401 "exx_bp_utils.f90"
             !
          END IF
       END DO
    END DO
!# 1406 "exx_bp_utils.f90"
    !
    ! send communication packets
    !
       ...
       DO iegrp=1, negrp
          DO iproc=0, nproc_egrp-1
             IF ( comm_send_reverse(iproc+1,iegrp,current_ik)%size.gt.0) THEN
                ...
                !
                tag = 0
!# 1437 "exx_bp_utils.f90"
                !
             END IF
          END DO
       END DO
    END IF
!# 1443 "exx_bp_utils.f90"
    !
    ! assign psi
    !
    DO iproc=0, nproc_egrp-1
...
!# 1455 "exx_bp_utils.f90"
    ...
    ! wait for everything to finish sending
    !
!# 1486 "exx_bp_utils.f90"
    !
  END SUBROUTINE transform_to_local
!# 1489 "exx_bp_utils.f90"
