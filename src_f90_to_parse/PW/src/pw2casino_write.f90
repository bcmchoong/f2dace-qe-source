...
!# 58 "pw2casino_write.f90"
   ...
   LOGICAL dotransform
!# 66 "pw2casino_write.f90"
   REAL(dp) :: av_overlap(5,2),avsq_overlap(5,2)
!# 68 "pw2casino_write.f90"
!----------------------------------------------------------------------------!
...
!# 81 "pw2casino_write.f90"
   dowrite=ionode.or..not.(gather.or.blip)
!# 83 "pw2casino_write.f90"
   ALLOCATE (idx (ngm) )
   ALLOCATE (igtog (ngm) )
   ALLOCATE (gtoig (ngm) )
   idx(:) = 0
   igtog(:) = 0
!# 89 "pw2casino_write.f90"
   IF( lsda )THEN
      nbndup = nbnd
      nbnddown = nbnd
      nk = nks/2
   ...
   ELSE
      ...
      !     nspin = 1
   ENDIF
!# 101 "pw2casino_write.f90"
   CALL calc_energies
!# 103 "pw2casino_write.f90"
   DO ispin = 1, nspin
      DO ik = 1, nk
         ikk = ik + nk*(ispin-1)
...
!# 110 "pw2casino_write.f90"
   ...
   DO ig = 1, ngm
      ...
      ENDIF
   ENDDO
!# 119 "pw2casino_write.f90"
   DEALLOCATE (idx)
!# 121 "pw2casino_write.f90"
   IF(dowrite)THEN
      IF(blip)THEN
         IF(binwrite)THEN
      ...
      WRITE (6,'(a)')
   ENDIF
!# 145 "pw2casino_write.f90"
   ALLOCATE ( g_l(3,ngtot_l), evc_l(ngtot_l) )
   DO ig = 1, ngtot_l
      g_l(:,ig) = g(:,igtog(ig))
   ENDDO
!# 150 "pw2casino_write.f90"
...
!# 319 "pw2casino_write.f90"
CONTAINS
!# 321 "pw2casino_write.f90"
                           ...
                           DO jh = ( ih + 1 ), nh(nt)
                              ...
                              ELSE
                                 enl=enl + &
                                    ...
                                    * wg(ibnd,ikk) * dvan(ih,jh,nt)
                              ENDIF
!# 424 "pw2casino_write.f90"
                           ENDDO
!# 426 "pw2casino_write.f90"
                        ENDDO
                        ijkb0 = ijkb0 + nh (nt)
                     ENDIF
                  ENDDO
         ...
         ENDDO
      ENDDO
!# 435 "pw2casino_write.f90"
      DEALLOCATE ( g2kin )
!# 443 "pw2casino_write.f90"
      eloc = eloc * omega
      !
...
!# 472 "pw2casino_write.f90"
      ...
      WRITE (stdout,*)
!# 488 "pw2casino_write.f90"
   END SUBROUTINE calc_energies
!# 491 "pw2casino_write.f90"
...
!# 611 "pw2casino_write.f90"
      IF(n_points_for_test<=0)RETURN
      IF(n_overlap_tests<=0)RETURN
!# 614 "pw2casino_write.f90"
...
!# 617 "pw2casino_write.f90"
      IF(.not.ionode)RETURN
      IF(blipreal==1.and.whichband==2)RETURN
!# 620 "pw2casino_write.f90"
...
!# 634 "pw2casino_write.f90"
   FUNCTION to_c80(c)
      CHARACTER(*),INTENT(in) :: c
      CHARACTER(80) :: to_c80
      to_c80=c
   END FUNCTION to_c80
!# 640 "pw2casino_write.f90"
   SUBROUTINE write_header
      INTEGER j, na, nt, at_num
      REAL(dp) :: kvec(3,nk),ksq(nk),kprod(6,nk)
!# 644 "pw2casino_write.f90"
...
!# 733 "pw2casino_write.f90"
         WRITE(iob)single_precision_blips                    ! single_precision_blips
!# 735 "pw2casino_write.f90"
         ! IF(no_loc_orbs>0)THEN
         !    ...
         ! ENDIF
!# 739 "pw2casino_write.f90"
         WRITE(iob)&
          (0,j=1,nbnd*nk*2) ,&
          (0,j=1,nbnd*nk*2) ,&
          (0,j=1,nbnd*nk*2) ,&
          (0,j=1,nbnd*nk*2)
!# 745 "pw2casino_write.f90"
...
!# 752 "pw2casino_write.f90"
      ...
      WRITE(io,'(a)') ' '
!# 807 "pw2casino_write.f90"
  100 FORMAT (3(1x,f20.15))
!# 809 "pw2casino_write.f90"
   END SUBROUTINE write_header
!# 812 "pw2casino_write.f90"
   SUBROUTINE write_gvecs(g,indx)
      REAL(DP),INTENT(in) :: g(:,:)
      INTEGER,INTENT(in) :: indx(:)
      INTEGER ig
!# 817 "pw2casino_write.f90"
      IF(binwrite)RETURN
!# 819 "pw2casino_write.f90"
      ...
      ENDDO
!# 829 "pw2casino_write.f90"
      WRITE(io,'(a)') ' '
   END SUBROUTINE write_gvecs
!# 833 "pw2casino_write.f90"
   SUBROUTINE write_gvecs_blip
      IF(binwrite)RETURN
!# 836 "pw2casino_write.f90"
      WRITE(io,'(a)') ' G VECTORS'
      WRITE(io,'(a)') ' ---------'
      WRITE(io,'(a)') ' Number of G-vectors'
      WRITE(io,*) 0
...
!# 1091 "pw2casino_write.f90"
   CHARACTER(20) FUNCTION i2s(n)
      INTEGER,INTENT(in) :: n
      INTEGER m,j
!# 1095 "pw2casino_write.f90"
      m = abs(n)
      DO j=len(i2s),2,-1
         ...
         IF(m==0)exit
      ENDDO
!# 1102 "pw2casino_write.f90"
      IF(n<0)THEN
         j = j-1
         i2s(j:j)='-'
      ENDIF
!# 1107 "pw2casino_write.f90"
      i2s=i2s(j:len(i2s))
   END FUNCTION i2s
!# 1111 "pw2casino_write.f90"
...
!# 1199 "pw2casino_write.f90"
   END FUNCTION write_mean
!# 1202 "pw2casino_write.f90"
