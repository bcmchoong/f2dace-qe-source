...
!# 21 "esm_hartree_mod.f90"
    ...
    END IF
!# 33 "esm_hartree_mod.f90"
  END SUBROUTINE esm_hartree
!# 35 "esm_hartree_mod.f90"
!
!-----------------------------------------------------------------------
...
!# 187 "esm_hartree_mod.f90"
      ...
      ENDDO
      ! end smoothing
!# 221 "esm_hartree_mod.f90"
      CALL cft_1z(vg_r, 1, dfftp%nr3, dfftp%nr3, -1, vg)
      vg3(:, ng_2d) = (vg3(:, ng_2d) + vg(:))*e2 ! factor e2: hartree -> Ry.
!# 224 "esm_hartree_mod.f90"
      DEALLOCATE (vg, vg_r)
    ENDIF ! IF( ng_2d > 0 )
!# 227 "esm_hartree_mod.f90"
! Hartree Energy
    ehart = 0.d0
    eh = 0d0
    DO ng_2d = 1, ngm_2d
...
! Map to FFT mesh (dfftp%nrx)
    ...
    DO ng = 1, ngm
      ...
      aux(ng) = vg3(n3,ng_2d)
    ENDDO
!# 258 "esm_hartree_mod.f90"
    DEALLOCATE (rhog3, vg3)
!# 260 "esm_hartree_mod.f90"
    RETURN
  END SUBROUTINE esm_hartree_bc1
!# 263 "esm_hartree_mod.f90"
  SUBROUTINE esm_hartree_bc2(rhog, ehart, aux)
!# 265 "esm_hartree_mod.f90"
    USE constants,        ONLY : tpi, fpi, e2
    USE gvect,            ONLY : ngm, mill
    USE cell_base,        ONLY : omega, alat, tpiba2, at, bg
    USE control_flags,    ONLY : gamma_only
...
!# 407 "esm_hartree_mod.f90"
      ...
      ENDDO
      ! end smoothing
!# 445 "esm_hartree_mod.f90"
      CALL cft_1z(vg_r, 1, dfftp%nr3, dfftp%nr3, -1, vg)
      vg3(:, ng_2d) = (vg3(:, ng_2d) + vg(:))*e2 ! factor e2: hartree -> Ry.
!# 448 "esm_hartree_mod.f90"
      DEALLOCATE (vg, vg_r)
    ENDIF ! IF( ng_2d > 0 )
!# 451 "esm_hartree_mod.f90"
! Hartree Energy
    ehart = 0.d0
    eh = 0d0
    DO ng_2d = 1, ngm_2d
...
! Map to FFT mesh (dfftp%nrx)
    ...
    DO ng = 1, ngm
      ...
      aux(ng) = vg3(n3,ng_2d)
    ENDDO
!# 482 "esm_hartree_mod.f90"
    DEALLOCATE (rhog3, vg3)
!# 484 "esm_hartree_mod.f90"
    RETURN
  END SUBROUTINE esm_hartree_bc2
!# 487 "esm_hartree_mod.f90"
  SUBROUTINE esm_hartree_bc3(rhog, ehart, aux)
!# 489 "esm_hartree_mod.f90"
    USE constants,        ONLY : tpi, fpi, e2
    USE gvect,            ONLY : ngm, mill
    USE cell_base,        ONLY : omega, alat, tpiba2, at, bg
    USE control_flags,    ONLY : gamma_only
...
!# 569 "esm_hartree_mod.f90"
      ...
      ENDDO
!# 584 "esm_hartree_mod.f90"
      CALL cft_1z(vg_r, 1, dfftp%nr3, dfftp%nr3, -1, vg)
      vg3(:, ng_2d) = (vg3(:, ng_2d) + vg(:))*e2 ! factor e2: hartree -> Ry.
    ENDDO
    DEALLOCATE (vg, vg_r)
!# 589 "esm_hartree_mod.f90"
!****For gp=0 case ********************
    ng_2d = imill_2d(0, 0)
    IF (ng_2d > 0) THEN
      ALLOCATE (vg(dfftp%nr3), vg_r(dfftp%nr3))
...
!# 627 "esm_hartree_mod.f90"
      ...
      ENDDO
      ! end smoothing
!# 659 "esm_hartree_mod.f90"
      CALL cft_1z(vg_r, 1, dfftp%nr3, dfftp%nr3, -1, vg)
      vg3(:, ng_2d) = (vg3(:, ng_2d) + vg(:))*e2 ! factor e2: hartree -> Ry.
!# 662 "esm_hartree_mod.f90"
      DEALLOCATE (vg, vg_r)
    ENDIF ! IF( ng_2d > 0 )
!# 665 "esm_hartree_mod.f90"
! Hartree Energy
    ehart = 0.d0
    eh = 0d0
    DO ng_2d = 1, ngm_2d
...
! Map to FFT mesh (dfftp%nrx)
    ...
    DO ng = 1, ngm
      ...
      aux(ng) = vg3(n3,ng_2d)
    ENDDO
!# 696 "esm_hartree_mod.f90"
    DEALLOCATE (rhog3, vg3)
!# 698 "esm_hartree_mod.f90"
    RETURN
  END SUBROUTINE esm_hartree_bc3
!# 701 "esm_hartree_mod.f90"
  SUBROUTINE esm_hartree_bc4(rhog, ehart, aux)
!# 703 "esm_hartree_mod.f90"
...
!# 856 "esm_hartree_mod.f90"
      DO igz = -(dfftp%nr3 - 1)/2, (dfftp%nr3 - 1)/2
        ...
        tmp4 = tmp4 + (0.d0, 0.d0)
!# 872 "esm_hartree_mod.f90"
        vg(iz) = fpi*rg3/(kn**2)
        ! bc4
        vr(iz) = fpi*rg3/(kn**2 + ci*2.d0*aaa*kn)
!# 876 "esm_hartree_mod.f90"
        !for smoothing
        c_r = cos(kn*z_r)
        s_r = sin(kn*z_r)
        c_l = cos(kn*z_l)
        ...
        !
      ENDDO
!# 889 "esm_hartree_mod.f90"
      CALL cft_1z(vg, 1, dfftp%nr3, dfftp%nr3, 1, vg_r)
      ! bc4
      CALL cft_1z(vr, 1, dfftp%nr3, dfftp%nr3, 1, vr_r)
!# 893 "esm_hartree_mod.f90"
      rg3 = rhog3(1, ng_2d)
      DO iz = 1, dfftp%nr3
        k3 = iz - 1
        IF (k3 >= (dfftp%nr3 - dfftp%nr3/2)) THEN
...
!# 918 "esm_hartree_mod.f90"
      ...
      DO iz = nz_r, nz_l
        ...
        vg_r(iz) = (a0 + a1*z + a2*z**2 + a3*z**3)
      ENDDO
!# 956 "esm_hartree_mod.f90"
      CALL cft_1z(vg_r, 1, dfftp%nr3, dfftp%nr3, -1, vg)
!# 958 "esm_hartree_mod.f90"
      vg3(:, ng_2d) = vg(:)*e2 ! factor e2: hartree -> Ry.
!# 960 "esm_hartree_mod.f90"
      DEALLOCATE (vg, vg_r, vr, vr_r)
    ENDIF ! IF( ng_2d > 0 )
!# 963 "esm_hartree_mod.f90"
! Hartree Energy
    ehart = 0.d0
    eh = 0d0
    DO ng_2d = 1, ngm_2d
...
! Map to FFT mesh (dfftp%nrx)
    ...
    DO ng = 1, ngm
      ...
      aux(ng) = vg3(n3,ng_2d)
    ENDDO
!# 994 "esm_hartree_mod.f90"
    DEALLOCATE (rhog3, vg3)
!# 996 "esm_hartree_mod.f90"
    RETURN
  END SUBROUTINE esm_hartree_bc4
!# 999 "esm_hartree_mod.f90"
