...
!# 10 "esm_force_mod.f90"
  IMPLICIT NONE
!# 12 "esm_force_mod.f90"
CONTAINS
!# 14 "esm_force_mod.f90"
...
!# 74 "esm_force_mod.f90"
    ...
    END IF
!# 86 "esm_force_mod.f90"
  END SUBROUTINE esm_force_ewr
!# 88 "esm_force_mod.f90"
...
!# 95 "esm_force_mod.f90"
    ...
    END IF
!# 107 "esm_force_mod.f90"
  END SUBROUTINE esm_force_ewg
!# 109 "esm_force_mod.f90"
...
!# 117 "esm_force_mod.f90"
    ...
    END IF
!# 129 "esm_force_mod.f90"
  END SUBROUTINE esm_force_lc
!# 131 "esm_force_mod.f90"
!-----------------------------------------------------------------------
...
!# 162 "esm_force_mod.f90"
    tmp = sqrt(alpha_g)
    rmax0 = 5.d0/tmp/alat
!# 165 "esm_force_mod.f90"
    ip = mp_rank(intra_bgrp_comm)
    np = mp_size(intra_bgrp_comm)
!# 168 "esm_force_mod.f90"
    ...
    DEALLOCATE (force)
!# 193 "esm_force_mod.f90"
  END SUBROUTINE esm_force_ewr_pbc
!# 195 "esm_force_mod.f90"
...
!# 422 "esm_force_mod.f90"
    forceion(:, :) = 0.d0
!# 424 "esm_force_mod.f90"
    ! same of the GSUM part in force_ew.f90
    ALLOCATE (aux(ngm))
    aux(:) = (0.d0, 0.d0)
!# 428 "esm_force_mod.f90"
    DO nt = 1, nsp
      DO ig = gstart, ngm
...
!# 509 "esm_force_mod.f90"
        ...
        ENDIF
!# 535 "esm_force_mod.f90"
      ENDDO
    ENDDO
    for_g(:, :) = for_g(:, :) + for(:, :)
!# 539 "esm_force_mod.f90"
    for_g(:, :) = for_g(:, :)*e2 ! factor e2: hartree -> Ry.
!# 541 "esm_force_mod.f90"
    DO it1 = 1, nat
    ...
    ENDDO
!# 547 "esm_force_mod.f90"
    RETURN
  END SUBROUTINE esm_force_ewg_bc1
!# 550 "esm_force_mod.f90"
  SUBROUTINE esm_force_ewg_bc2(alpha_g, forceion)
!# 552 "esm_force_mod.f90"
    USE constants,        ONLY : tpi, fpi, e2
    USE cell_base,        ONLY : omega, alat, tpiba2, at, bg
    USE control_flags,    ONLY : gamma_only
    USE ions_base,        ONLY : zv, nat, nsp, tau, ityp
    USE gvect,            ONLY : gstart, ngm, g
!# 558 "esm_force_mod.f90"
    IMPLICIT NONE
    REAL(DP), INTENT(in)    :: alpha_g
    REAL(DP), INTENT(out)   :: forceion(3, nat)
    !
...
!# 598 "esm_force_mod.f90"
        ...
        IF (gstart == 2) THEN
          for(3, it2) = for(3, it2) + t2_for*(kk1_for + kk2_for)
        ENDIF
!# 635 "esm_force_mod.f90"
      ENDDO
    ENDDO
    for_g(:, :) = for_g(:, :) + for(:, :)
!# 639 "esm_force_mod.f90"
    for_g(:, :) = for_g(:, :)*e2 ! factor e2: hartree -> Ry.
!# 641 "esm_force_mod.f90"
    DO it1 = 1, nat
      forceion(1, it1) = -sum(for_g(1:2, it1)*bg(1, 1:2))*sqrt(tpiba2)
      forceion(2, it1) = -sum(for_g(1:2, it1)*bg(2, 1:2))*sqrt(tpiba2)
      forceion(3, it1) = -for_g(3, it1)
      ...
      ENDIF
    ENDDO
!# 651 "esm_force_mod.f90"
    RETURN
  END SUBROUTINE esm_force_ewg_bc2
!# 654 "esm_force_mod.f90"
  SUBROUTINE esm_force_ewg_bc3(alpha_g, forceion)
!# 656 "esm_force_mod.f90"
    USE constants,        ONLY : tpi, fpi, e2
    USE cell_base,        ONLY : omega, alat, tpiba2, at, bg
    USE control_flags,    ONLY : gamma_only
    USE ions_base,        ONLY : zv, nat, nsp, tau, ityp
    USE gvect,            ONLY : gstart, ngm, g
!# 662 "esm_force_mod.f90"
    IMPLICIT NONE
    REAL(DP), INTENT(in)    :: alpha_g
    REAL(DP), INTENT(out)   :: forceion(3, nat)
    !
...
!# 701 "esm_force_mod.f90"
        ...
        IF (gstart == 2) THEN
          for(3, it2) = for(3, it2) + t2_for*(kk1_for + kk2_for)
        ENDIF
!# 731 "esm_force_mod.f90"
      ENDDO
    ENDDO
    for_g(:, :) = for_g(:, :) + for(:, :)
!# 735 "esm_force_mod.f90"
    for_g(:, :) = for_g(:, :)*e2 ! factor e2: hartree -> Ry.
!# 737 "esm_force_mod.f90"
    DO it1 = 1, nat
      forceion(1, it1) = -sum(for_g(1:2, it1)*bg(1, 1:2))*sqrt(tpiba2)
      forceion(2, it1) = -sum(for_g(1:2, it1)*bg(2, 1:2))*sqrt(tpiba2)
      forceion(3, it1) = -for_g(3, it1)
    ENDDO
!# 743 "esm_force_mod.f90"
    RETURN
  END SUBROUTINE esm_force_ewg_bc3
!# 746 "esm_force_mod.f90"
  SUBROUTINE esm_force_ewg_bc4(alpha_g, forceion)
!# 748 "esm_force_mod.f90"
    USE constants,        ONLY : tpi, fpi, e2
    USE cell_base,        ONLY : omega, alat, tpiba2, at, bg
    USE control_flags,    ONLY : gamma_only
    USE ions_base,        ONLY : zv, nat, nsp, tau, ityp
    USE gvect,            ONLY : gstart, ngm, g
!# 754 "esm_force_mod.f90"
    IMPLICIT NONE
    REAL(DP), INTENT(in)    :: alpha_g
    REAL(DP), INTENT(out)   :: forceion(3, nat)
    !
    ...
    DO it1 = 1, nat
      DO it2 = 1, nat
        ...
        IF (gstart == 2) THEN
          for(3, it2) = for(3, it2) + t2_for*(kk1_for + kk2_for)
        ENDIF
!# 969 "esm_force_mod.f90"
      ENDDO
    ENDDO
    for_g(:, :) = for_g(:, :) + for(:, :)
!# 973 "esm_force_mod.f90"
    for_g(:, :) = for_g(:, :)*e2 ! factor e2: hartree -> Ry.
!# 975 "esm_force_mod.f90"
    DO it1 = 1, nat
      forceion(1, it1) = -sum(for_g(1:2, it1)*bg(1, 1:2))*sqrt(tpiba2)
      forceion(2, it1) = -sum(for_g(1:2, it1)*bg(2, 1:2))*sqrt(tpiba2)
      forceion(3, it1) = -for_g(3, it1)
    ENDDO
!# 981 "esm_force_mod.f90"
    RETURN
  END SUBROUTINE esm_force_ewg_bc4
!# 984 "esm_force_mod.f90"
!-----------------------------------------------------------------------
...
! Map to FFT mesh
    ...
    DO ng = 1, ngm
      ...
      ENDIF
    ENDDO
!# 1039 "esm_force_mod.f90"
    L = at(3, 3)*alat
    sa = omega/L
    z0 = L/2.d0
    tmp = 1.d0
    z1 = z0 + esm_w
!# 1045 "esm_force_mod.f90"
    ALLOCATE (for_g(3, nat))
    for_g(:, :) = 0.d0
!# 1048 "esm_force_mod.f90"
!**** for gp!=0 *********
    ...
    DO ng_2d = 1, ngm_2d
      ...
      k2 = mill_2d(2, ng_2d)
      IF (k1 == 0 .and. k2 == 0) CYCLE
!# 1057 "esm_force_mod.f90"
      t(1:2) = k1*bg(1:2, 1) + k2*bg(1:2, 2)
      gp2 = sum(t(:)*t(:))*tpiba2
      gp = sqrt(gp2)
!# 1061 "esm_force_mod.f90"
      DO it = 1, nat
        IF (gamma_only) THEN
...
!# 1146 "esm_force_mod.f90"
    ...
    ENDDO
!# 1155 "esm_force_mod.f90"
    DEALLOCATE (for_g)
    DEALLOCATE (rhog3)
!# 1158 "esm_force_mod.f90"
    RETURN
  END SUBROUTINE esm_force_lc_bc1
!# 1161 "esm_force_mod.f90"
  SUBROUTINE esm_force_lc_bc2(aux, forcelc)
!# 1163 "esm_force_mod.f90"
    USE constants,        ONLY : tpi, fpi, e2
    USE gvect,            ONLY : ngm, mill
    USE cell_base,        ONLY : omega, alat, tpiba2, at, bg
    USE control_flags,    ONLY : gamma_only
...
! Map to FFT mesh
    ...
    DO ng = 1, ngm
      ...
      ENDIF
    ENDDO
!# 1203 "esm_force_mod.f90"
    L = at(3, 3)*alat
    sa = omega/L
    z0 = L/2.d0
    tmp = 1.d0
    z1 = z0 + esm_w
!# 1209 "esm_force_mod.f90"
    ALLOCATE (for_g(3, nat))
    for_g(:, :) = 0.d0
!# 1212 "esm_force_mod.f90"
!**** for gp!=0 *********
    ALLOCATE (for(3, nat), vg_f(dfftp%nr3x, 3), vg_f_r(dfftp%nr3x, 3))
    for(:, :) = 0.d0
    vg_f_r(:, :) = (0.d0, 0.d0)
    DO ng_2d = 1, ngm_2d
      ...
      k2 = mill_2d(2, ng_2d)
      IF (k1 == 0 .and. k2 == 0) CYCLE
!# 1221 "esm_force_mod.f90"
      t(1:2) = k1*bg(1:2, 1) + k2*bg(1:2, 2)
      gp2 = sum(t(:)*t(:))*tpiba2
      gp = sqrt(gp2)
!# 1225 "esm_force_mod.f90"
      DO it = 1, nat
        IF (gamma_only) THEN
          tt = -fpi*zv(ityp(it))/sa*2.d0
        ELSE
...
!***** for gp==0********
    ...
    IF (ng_2d > 0) THEN
      ...
      DEALLOCATE (vg_f, vg_f_r)
    ENDIF ! IF( ng_2d > 0 )
!# 1315 "esm_force_mod.f90"
!***** sum short_range part and long_range part in local potential force
!***** at cartecian coordinate
!# 1318 "esm_force_mod.f90"
    DO it = 1, nat
      ! factor e2: hartree -> Ry.
      forcelc(1, it) = forcelc(1, it) &
                       + sum(for_g(1:2, it)*bg(1, 1:2))*sqrt(tpiba2)*omega*e2
      ...
      forcelc(3, it) = forcelc(3, it) + for_g(3, it)*omega*e2
    ENDDO
!# 1327 "esm_force_mod.f90"
    DEALLOCATE (for_g)
    DEALLOCATE (rhog3)
!# 1330 "esm_force_mod.f90"
    RETURN
  END SUBROUTINE esm_force_lc_bc2
!# 1333 "esm_force_mod.f90"
  SUBROUTINE esm_force_lc_bc3(aux, forcelc)
!# 1335 "esm_force_mod.f90"
    USE constants,        ONLY : tpi, fpi, e2
    USE gvect,            ONLY : ngm, mill
    USE cell_base,        ONLY : omega, alat, tpiba2, at, bg
    USE control_flags,    ONLY : gamma_only
...
! Map to FFT mesh
    ...
    DO ng = 1, ngm
      ...
      ENDIF
    ENDDO
!# 1374 "esm_force_mod.f90"
    L = at(3, 3)*alat
    sa = omega/L
    z0 = L/2.d0
    tmp = 1.d0
    z1 = z0 + esm_w
!# 1380 "esm_force_mod.f90"
    ALLOCATE (for_g(3, nat))
    for_g(:, :) = 0.d0
!# 1383 "esm_force_mod.f90"
!**** for gp!=0 *********
    ALLOCATE (for(3, nat), vg_f(dfftp%nr3x, 3), vg_f_r(dfftp%nr3x, 3))
    for(:, :) = 0.d0
    vg_f_r(:, :) = (0.d0, 0.d0)
    DO ng_2d = 1, ngm_2d
      ...
      k2 = mill_2d(2, ng_2d)
      IF (k1 == 0 .and. k2 == 0) CYCLE
!# 1392 "esm_force_mod.f90"
      t(1:2) = k1*bg(1:2, 1) + k2*bg(1:2, 2)
      gp2 = sum(t(:)*t(:))*tpiba2
      gp = sqrt(gp2)
!# 1396 "esm_force_mod.f90"
      DO it = 1, nat
        IF (gamma_only) THEN
          tt = -fpi*zv(ityp(it))/sa*2.d0
        ELSE
...
!***** for gp==0********
    ...
    IF (ng_2d > 0) THEN
      ...
      DO it = 1, nat
        ...
        ENDDO
      ENDDO
!# 1478 "esm_force_mod.f90"
      DEALLOCATE (vg_f, vg_f_r)
    ENDIF ! IF( ng_2d > 0 )
!# 1481 "esm_force_mod.f90"
!***** sum short_range part and long_range part in local potential force
!***** at cartecian coordinate
!# 1484 "esm_force_mod.f90"
    DO it = 1, nat
                                             ! factor e2: hartree -> Ry.
      forcelc(1, it) = forcelc(1, it) &
                       + sum(for_g(1:2, it)*bg(1, 1:2))*sqrt(tpiba2)*omega*e2
      ...
      forcelc(3, it) = forcelc(3, it) + for_g(3, it)*omega*e2
    ENDDO
!# 1493 "esm_force_mod.f90"
    DEALLOCATE (for_g)
    DEALLOCATE (rhog3)
!# 1496 "esm_force_mod.f90"
    RETURN
  END SUBROUTINE esm_force_lc_bc3
!# 1499 "esm_force_mod.f90"
  SUBROUTINE esm_force_lc_bc4(aux, forcelc)
!# 1501 "esm_force_mod.f90"
    USE constants,        ONLY : tpi, fpi, e2
    USE gvect,            ONLY : ngm, mill
    USE cell_base,        ONLY : omega, alat, tpiba2, at, bg
    USE control_flags,    ONLY : gamma_only
...
!# 1544 "esm_force_mod.f90"
    ...
    z1 = z0 + esm_w
    aaa = esm_a
!# 1551 "esm_force_mod.f90"
    ALLOCATE (for_g(3, nat))
    for_g(:, :) = 0.d0
!# 1554 "esm_force_mod.f90"
!**** for gp!=0 *********
    ALLOCATE (for(3, nat), vg_f(dfftp%nr3x, 3), vg_f_r(dfftp%nr3x, 3))
    for(:, :) = 0.d0
    vg_f_r(:, :) = (0.d0, 0.d0)
    DO ng_2d = 1, ngm_2d
      ...
      k2 = mill_2d(2, ng_2d)
      IF (k1 == 0 .and. k2 == 0) CYCLE
!# 1563 "esm_force_mod.f90"
      t(1:2) = k1*bg(1:2, 1) + k2*bg(1:2, 2)
      gp2 = sum(t(:)*t(:))*tpiba2
      gp = sqrt(gp2)
!# 1567 "esm_force_mod.f90"
      DO it = 1, nat
        IF (gamma_only) THEN
          tt = -fpi*zv(ityp(it))/sa*2.d0
        ELSE
...
!***** for gp==0********
    ...
    IF (ng_2d > 0) THEN
      ...
      DEALLOCATE (vg_f, vg_f_r)
    ENDIF ! IF( ng_2d > 0 )
!# 1700 "esm_force_mod.f90"
!***** sum short_range part and long_range part in local potential force
!***** at cartecian coordinate
!# 1703 "esm_force_mod.f90"
    DO it = 1, nat
                                             ! factor e2: hartree -> Ry.
      forcelc(1, it) = forcelc(1, it) &
                       + sum(for_g(1:2, it)*bg(1, 1:2))*sqrt(tpiba2)*omega*e2
      ...
      forcelc(3, it) = forcelc(3, it) + for_g(3, it)*omega*e2
    ENDDO
!# 1712 "esm_force_mod.f90"
    DEALLOCATE (for_g)
    DEALLOCATE (rhog3)
!# 1715 "esm_force_mod.f90"
    RETURN
