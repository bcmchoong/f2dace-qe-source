...
!     do l = 1, nactive  ! THIS COULD/SHOULD BE A GLOBAL CALL (ONLY WITHIN ( 1.D0, 0.D0 ) BGRP THOUGH)
...
!# 262 "bpcg_k.f90"
     CALL g_psi( npwx, npw, nactive, npol, z, e(done+1) )
!# 264 "bpcg_k.f90"
  !- project on conduction bands
     CALL start_clock( 'pcg:ortho' )
     !$acc host_data use_device(spsi0, psi0, z, spsi0vec)
     CALL MYZGEMM( 'C', 'N', nbnd, nactive, kdim, ( 1.D0, 0.D0 ), spsi0, kdmx, z, kdmx, ( 0.D0, 0.D0 ), spsi0vec, nbnd )
     CALL mp_sum( spsi0vec, intra_bgrp_comm )
     CALL MYZGEMM( 'N', 'N', kdim, nactive, nbnd, (-1.D0,0.D0), psi0, kdmx, spsi0vec, nbnd, ( 1.D0, 0.D0 ), z, kdmx )
     !$acc end host_data
     CALL stop_clock( 'pcg:ortho' )
  !-
     !$acc parallel loop private(i) 
