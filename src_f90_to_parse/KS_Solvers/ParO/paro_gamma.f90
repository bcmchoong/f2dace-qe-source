...
!
!-------------------------------------------------------------------------------
SUBROUTINE paro_gamma( h_psi_ptr, s_psi_ptr, hs_1psi_ptr, g_psi_ptr, overlap, &
...
!  INTEGER, INTENT(IN)        :: paro_flag
  ...
  !------------------------------------------------------------------------
  EXTERNAL h_psi, s_psi, hs_1psi, g_psi
  ...
  ! subroutine g_psi_ptr (npwx,npw,psi,eig)       computes g*psi  for a single band
!# 74 "paro_gamma.f90"
  !
  ! ... local variables
  !
...
!# 111 "paro_gamma.f90"
     ...
     call s_psi  (npwx,npw,nbnd,psi2,evc) ! computes S*psi needed to orthogonalize to nbase
     lbnd = nbase
...
!# 132 "paro_gamma.f90"
     ...
     DO ibnd=ibnd_start,ibnd_end
        ...
        CALL pcg_gamma(hs_1psi, g_psi, psi2, evc, npw, npwx, nbnd, psi2(:,nbase+ibnd), ethr, iter, eig(ibnd), nhpsi)
     END DO
...
!# 145 "paro_gamma.f90"
        CALL rotate_wfc_gamma ( h_psi, s_psi, overlap, npwx, npw, ndiag, ndiag, psi2, psi2, ew )
!# 152 "paro_gamma.f90"
