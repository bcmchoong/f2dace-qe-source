...
!
!----------------------------------------------------------------------------
SUBROUTINE pcg_k( hs_1psi_ptr, g_psi_ptr, psi0, spsi0, npw, npwx, nbnd, npol, psi, ethr, iter, e, nhpsi )
  !----------------------------------------------------------------------------
  !
...
!# 91 "pcg_k.f90"
  ...
  z(:) = r(:) ; call g_psi(npwx,npw,z,e)    ! initial preconditioned gradient
!- project on conduction bands
  CALL start_clock( 'pcg:ortho' )
  CALL ZGEMV( 'C', kdim, nbnd, ( 1.D0, 0.D0 ), spsi0, kdmx,       z, 1, ( 0.D0, 0.D0 ), spsi0vec, 1 )
  CALL mp_sum( spsi0vec, intra_bgrp_comm )
  CALL ZGEMV( 'N', kdim, nbnd, (-1.D0,0.D0), psi0, kdmx, spsi0vec, 1, ( 1.D0, 0.D0 ),        z, 1 )
...
!# 149 "pcg_k.f90"
     ...
     z(:) = r(:) ; call g_psi(npwx,npw,z,e) ! updated preconditioned gradient
!- project on conduction bands
  CALL start_clock( 'pcg:ortho' )
  CALL ZGEMV( 'C', kdim, nbnd, ( 1.D0, 0.D0 ), spsi0, kdmx,       z, 1, ( 0.D0, 0.D0 ), spsi0vec, 1 )
  CALL mp_sum( spsi0vec, intra_bgrp_comm )
