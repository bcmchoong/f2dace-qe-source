...
SUBROUTINE crmmdiagg( h_psi_ptr, s_psi_ptr, npwx, npw, nbnd, npol, psi, hpsi, spsi, e, &
  ...
  IF ( ibnd_start > 1 ) THEN
     !
     !$acc kernels
     psi (:,1:(ibnd_start-1)) = ( 0._DP, 0._DP )
     hpsi(:,1:(ibnd_start-1)) = ( 0._DP, 0._DP )
     IF ( uspp ) &
     spsi(:,1:(ibnd_start-1)) = ( 0._DP, 0._DP )
     !$acc end kernels
     !
  END IF
  !
  IF ( ibnd_end < nbnd ) THEN
     !
     !$acc kernels
     psi (:,(ibnd_end+1):nbnd) = ( 0._DP, 0._DP )
     hpsi(:,(ibnd_end+1):nbnd) = ( 0._DP, 0._DP )
     IF ( uspp ) &
     spsi(:,(ibnd_end+1):nbnd) = ( 0._DP, 0._DP )
     !$acc end kernels
     !
  END IF
  !
...
CONTAINS
  ...
  SUBROUTINE do_diis( idiis )
    ...
    DO ibnd = ibnd_start, ibnd_end
       ...
       CALL MYZGEMV( 'C', kdim, idiis, ( 1._DP, 0._DP ), vec2(1,1), kdmx, &
                   vec1(1), 1, ( 0._DP, 0._DP ), tc(1,jbnd), 1 )
       !$acc end host_data
       !
    END DO
    !
    ...
    DO ibnd = ibnd_start, ibnd_end
       ...
       IF ( idiis > 1 ) THEN
          ...
          !
          !$acc kernels
          psi (:,ibnd) = ( 0._DP, 0._DP )
          hpsi(:,ibnd) = ( 0._DP, 0._DP )
          IF ( uspp ) spsi(:,ibnd) = ( 0._DP, 0._DP )
          kpsi(:,kbnd) = ( 0._DP, 0._DP )
          !$acc end kernels
          !
          DO kdiis = 1, idiis
             !
  ...
  SUBROUTINE diag_diis( ibnd, idiis, vc )
    ...
    h2 = ( 0._DP, 0._DP )
    !
    CALL ZGEMM( 'N', 'N', ndim, kdim, ndim, ( 1._DP, 0._DP ), h1, ndim, x1, ndim, ( 0._DP, 0._DP ), h2, ndim )
    !
    h3 = ( 0._DP, 0._DP )
    !
    CALL ZGEMM( 'C', 'N', kdim, kdim, ndim, ( 1._DP, 0._DP ), x1, ndim, h2, ndim, ( 0._DP, 0._DP ), h3, ndim )
    !
    e1 = 0._DP
    !
    ...
    END DO
    !
    CALL ZGEMV( 'N', ndim, kdim, ( 1._DP, 0._DP ), x1, ndim, h3(:,imin), 1, ( 0._DP, 0._DP ), vc, 1 )
    !
    s1(1:ndim,1:ndim) = sc(1:ndim,1:ndim,ibnd)
    !
    CALL ZGEMV( 'N', ndim, ndim, ( 1._DP, 0._DP ), s1, ndim, vc, 1, ( 0._DP, 0._DP ), u1, 1 )
    !
