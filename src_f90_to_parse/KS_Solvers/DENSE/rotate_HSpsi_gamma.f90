...
!# 119 "rotate_HSpsi_gamma.f90"
  CALL mp_type_free( column_type )
!# 121 "rotate_HSpsi_gamma.f90"
...
!# 160 "rotate_HSpsi_gamma.f90"
  IF (overlap) THEN
!# 162 "rotate_HSpsi_gamma.f90"
     if (n_start .le. n_end) &
...
!# 170 "rotate_HSpsi_gamma.f90"
  ELSE IF (present(spsi)) THEN
!# 172 "rotate_HSpsi_gamma.f90"
     CALL dev_memcpy(spsi, psi, [1, npwx], 1, [1,nbnd])
!# 174 "rotate_HSpsi_gamma.f90"
  END IF
  !
  !$acc end host_data
  !
...
!# 275 "rotate_HSpsi_gamma.f90"
  ALLOCATE( hh( nx, nx ), ss( nx, nx ), en( nstart ) )
!# 277 "rotate_HSpsi_gamma.f90"
  !
  ! ... Set up the Hamiltonian and Overlap matrix on the subspace :
  !
  ! ...      H_ij = <psi_i| H |psi_j>     S_ij = <psi_i| S |psi_j>
...
!  call start_clock('rotHSw:ev:b0'); CALL mp_barrier( ortho_parent_comm ); call stop_clock('rotHSw:ev:b0')
  ...
  !
!# 329 "rotate_HSpsi_gamma.f90"
  ALLOCATE ( v( nstart, my_nbnd ) )
!# 331 "rotate_HSpsi_gamma.f90"
  CALL collect_v (  v )
!# 333 "rotate_HSpsi_gamma.f90"
  CALL mp_type_create_column_section(psi(1,1), 0, npwx, npwx, column_type)
!# 335 "rotate_HSpsi_gamma.f90"
  ALLOCATE( aux( npwx, my_nbnd ) )
!# 337 "rotate_HSpsi_gamma.f90"
  ...
  END IF
!# 345 "rotate_HSpsi_gamma.f90"
  DEALLOCATE( aux )
!# 347 "rotate_HSpsi_gamma.f90"
  DEALLOCATE( v, vv )
  !
  CALL mp_type_free( column_type )
  !
  ...
  DEALLOCATE( rank_ip )
  call stop_clock('rotHSw'); !write(*,*) 'stop rotHSw' ; FLUSH(6)
!# 357 "rotate_HSpsi_gamma.f90"
  !call print_clock('rotHSw')
  !call print_clock('rotHSw:hc')
  !call print_clock('rotHSw:diag')
  !call print_clock('rotHSw:evc')
!# 362 "rotate_HSpsi_gamma.f90"
  RETURN
  !
CONTAINS
  !
...
!# 397 "rotate_HSpsi_gamma.f90"
           ! use blas subs. on the matrix block
!# 399 "rotate_HSpsi_gamma.f90"
