...
SUBROUTINE gram_schmidt_k( npwx, npw, nbnd, npol, psi, hpsi, spsi, e, &
  ...
  phi = ( 0._DP, 0._DP )
  !
  IF ( eigen_ ) hphi = ( 0._DP, 0._DP )
  !
  IF ( uspp )   sphi = ( 0._DP, 0._DP )
  !$acc end kernels
  !
  ! ... Set owers of blocks
  !
...
CONTAINS
  ...
  SUBROUTINE gram_schmidt_diag( ibnd_start, ibnd_end )
    ...
    DO ibnd = ibnd_start, ibnd_end
       ...
       IF ( ibnd > ibnd_start ) THEN
          ...
          IF ( uspp ) THEN
             !
             CALL MYZGEMV( 'C', kdim, ibnd - ibnd_start, ( 1._DP, 0._DP ), phi(1,ibnd_start), kdmx, &
                         spsi(1,ibnd), 1, ( 0._DP, 0._DP ), sc(1), 1 )
             !
          ELSE
             !
             CALL MYZGEMV( 'C', kdim, ibnd - ibnd_start, ( 1._DP, 0._DP ), phi(1,ibnd_start), kdmx, &
                         psi(1,ibnd), 1, ( 0._DP, 0._DP ), sc(1), 1 )
             !
          END IF
          !$acc end host_data
          !
          ...
          CALL MYZGEMV( 'N', kdim, ibnd - ibnd_start, (-1._DP, 0._DP ), phi(1,ibnd_start), kdmx, &
                      sc(1), 1, ( 1._DP, 0._DP ), phi(1,ibnd), 1 )
          !
          IF ( eigen_ ) &
          CALL MYZGEMV( 'N', kdim, ibnd - ibnd_start, (-1._DP, 0._DP ), hphi(1,ibnd_start), kdmx, &
                      sc(1), 1, ( 1._DP, 0._DP ), hphi(1,ibnd), 1 )
          !
          IF ( uspp ) &
          CALL MYZGEMV( 'N', kdim, ibnd - ibnd_start, (-1._DP, 0._DP ), sphi(1,ibnd_start), kdmx, &
                      sc(1), 1, ( 1._DP, 0._DP ), sphi(1,ibnd), 1 )
          !$acc end host_data 
          !
       END IF
       !
  ...
  SUBROUTINE project_offdiag( ibnd_start, ibnd_end, jbnd_start, jbnd_end )
    ...
    IF ( uspp ) THEN
       !
       CALL MYZGEMM( 'C', 'N', ibnd_size, jbnd_size, kdim, ( 1._DP, 0._DP ), phi(1,ibnd_start), kdmx, &
                   spsi(1,jbnd_start), kdmx, ( 0._DP, 0._DP ), sc2(1,1), nbsize )
       !
    ELSE
       !
       CALL MYZGEMM( 'C', 'N', ibnd_size, jbnd_size, kdim, ( 1._DP, 0._DP ), phi(1,ibnd_start), kdmx, &
                   psi(1,jbnd_start), kdmx, ( 0._DP, 0._DP ), sc2(1,1), nbsize )
       !
    END IF
    !
    ...
    !
    CALL MYZGEMM( 'N', 'N', kdim, jbnd_size, ibnd_size, (-1._DP, 0._DP ), phi(1,ibnd_start), kdmx, &
                sc2(1,1), nbsize, ( 1._DP, 0._DP ), phi(1,jbnd_start), kdmx )
    !
    IF ( eigen_ ) &
    CALL MYZGEMM( 'N', 'N', kdim, jbnd_size, ibnd_size, (-1._DP, 0._DP ), hphi(1,ibnd_start), kdmx, &
                sc2(1,1), nbsize, ( 1._DP, 0._DP ), hphi(1,jbnd_start), kdmx )
    !
    IF ( uspp ) &
    CALL MYZGEMM( 'N', 'N', kdim, jbnd_size, ibnd_size, (-1._DP, 0._DP ), sphi(1,ibnd_start), kdmx, &
                sc2(1,1), nbsize, ( 1._DP, 0._DP ), sphi(1,jbnd_start), kdmx )
    !
    !$acc end host_data
    !
    RETURN
