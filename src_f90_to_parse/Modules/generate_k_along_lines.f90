...
!# 54 "generate_k_along_lines.f90"
RETURN
END SUBROUTINE generate_k_along_lines
!# 57 "generate_k_along_lines.f90"
...
USE kinds, ONLY : DP
IMPLICIT NONE
!# 67 "generate_k_along_lines.f90"
INTEGER, INTENT(IN) :: nkaux, nkstot, wkaux(nkaux)
REAL(DP), INTENT(IN) :: xkaux(3,nkaux)
REAL(DP), INTENT(OUT) :: xk(3,nkstot), wk(nkstot)
!# 71 "generate_k_along_lines.f90"
REAL(DP) :: dkx(3), dky(3), wk0
INTEGER :: ijk, i, j
!# 74 "generate_k_along_lines.f90"
...
DO i=1, wkaux(2)
   ...
   ENDDO
ENDDO
!# 89 "generate_k_along_lines.f90"
RETURN
!# 91 "generate_k_along_lines.f90"
