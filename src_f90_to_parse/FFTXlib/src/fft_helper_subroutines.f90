...
!# 119 "fft_helper_subroutines.f90"
     ENDIF
     !
  ...
  SUBROUTINE tg_reduce_rho_3( rhos, tmp_rhos, desc )
     ...
     !
     IF ( desc%nproc2 > 1 ) THEN
!# 149 "fft_helper_subroutines.f90"
     ENDIF
     !
     !BRING CHARGE DENSITY BACK TO ITS ORIGINAL POSITION
     !
  ...
  SUBROUTINE tg_reduce_rho_4( rhos, tmp_rhos, desc )
     ...
     !
     IF ( desc%nproc2 > 1 ) THEN
!# 182 "fft_helper_subroutines.f90"
     ENDIF
     !
     !BRING CHARGE DENSITY BACK TO ITS ORIGINAL POSITION
     !
  ...
  SUBROUTINE tg_reduce_rho_5( rhos, tmp_rhos, desc )
     ...
     !
     IF ( desc%nproc2 > 1 ) THEN
!# 215 "fft_helper_subroutines.f90"
     ENDIF
     !
     !BRING CHARGE DENSITY BACK TO ITS ORIGINAL POSITION
     !
...
!# 445 "fft_helper_subroutines.f90"
  ...
  SUBROUTINE fftx_c2psi_k( desc, psi, c, igk, ngk, howmany )
     ...
     ELSE
        ...
        !$acc end kernels
        !
!# 495 "fft_helper_subroutines.f90"
        !$omp parallel
        !$omp do
!# 498 "fft_helper_subroutines.f90"
        DO ig = 1, ngk
          psi(desc%nl(igk(ig))) = c(ig,1)
        ENDDO
!# 502 "fft_helper_subroutines.f90"
        !$omp end do nowait
        !$omp end parallel
!# 505 "fft_helper_subroutines.f90"
        !
     ENDIF
     !
     !$acc end data
...
!# 732 "fft_helper_subroutines.f90"
     INTEGER :: ig
     !
     nl  => desc%nl_d
     nlm => desc%nlm_d
...
!# 836 "fft_helper_subroutines.f90"
        ...
        ELSEIF ( ieg == dbnd ) THEN
           ...
           ENDDO
        ENDIF
!# 864 "fft_helper_subroutines.f90"
        !$omp end task
!# 866 "fft_helper_subroutines.f90"
        !
     ENDDO
!# 872 "fft_helper_subroutines.f90"
     !$omp end single
     !$omp end parallel
!# 875 "fft_helper_subroutines.f90"
     !
     RETURN
     !
  ...
  SUBROUTINE fftx_c2psi_k_tg( desc, psis, c_bgrp, igk, ngk, dbnd )
     ...
     right_nnr = desc%nnr
     !
!# 903 "fft_helper_subroutines.f90"
     !
     ...
     !
!# 912 "fft_helper_subroutines.f90"
     !$omp parallel do collapse(2) private(js,je)
!# 916 "fft_helper_subroutines.f90"
     DO idx = 0, MIN(ntgrp-1,dbnd-1)
       ...
       ENDDO
     ENDDO
!# 924 "fft_helper_subroutines.f90"
     !$omp end parallel do
!# 929 "fft_helper_subroutines.f90"
     !
     RETURN
     !
...
!# 1047 "fft_helper_subroutines.f90"
